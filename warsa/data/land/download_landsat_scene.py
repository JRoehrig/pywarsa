...
"""
    Landsat Data hydro_cst_download from earth explorer
...
def connect_earthexplorer_proxy(proxy_info, usgs):
    ...
    if data.find('You must sign in as a registered user to hydro_cst_download data or place orders for USGS EROS products') > 0:
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        print "example (scene): python %s -z unzip -b LT5 -o scene -d 20101001 -f 20101231 -s 203034 -u usgs.txt --output /outputdir/" % \
              sys.argv[0]
        print "example (scene): python %s -z unzip -b LT5 -o scene -d 20101001 -f 20101231 -s 203034 -u usgs.txt --output /outputdir/ -k update --outputcatalogs /outputcatalogsdir/" % \
              sys.argv[0]
        print "example (scene): python %s -b LE7 -o scene -d 20141201 -f 20141231 -s 191025 -u usgs.txt --output . --dir=3373 --station SG1" % \
              sys.argv[0]
        ...
        parser.add_option("--output", dest="output", action="store", type="string", \
                          help="Where to hydro_cst_download files", default='/tmp/LANDSAT')
        parser.add_option("--outputcatalogs", dest="outputcatalogs", action="store", type="string", \
                          help="Where to hydro_cst_download metadata catalog files", default='/tmp/LANDSAT')
        ...
        while (curr_date < date_end) and check == 1:
            ...
            for station in stations:
                for version in ['00', '01', '02']:
                    ...
                    url = "http://earthexplorer.usgs.gov/hydro_cst_download/%s/%s/STANDARD/EE" % (repert, nom_prod)
                    print url
        ...
        if os.path.exists(lsdestdir):
            ...
            while check == 1:
                for collectionid in repert:
                    url = "http://earthexplorer.usgs.gov/hydro_cst_download/%s/%s/STANDARD/EE" % (collectionid, nom_prod)
                    try:
                        downloadChunks(url, "%s" % rep_scene, nom_prod + '.tgz')
                    except:
                        print '   product %s not found' % nom_prod
    ...
    if options.option == 'liste':
        ...
        for ligne in lignes:
            ...
            if not os.path.exists(rep + '/' + site):
                os.mkdir(rep + '/' + site)
            url = "http:///earthexplorer.usgs.gov/hydro_cst_download/%s/%s/STANDARD/EE" % (repert, produit)
            print 'url=', url
            try:
                if options.proxy != None:
                    connect_earthexplorer_proxy(proxy, usgs)
