...
class GPMImerg3BHHRv05FTP(object):
    """
    ...
    def __init__(self, local_folder, ftp_user, ftp_password):
        ...
        ftp_dir = '/gpmallversions/V05/'
        self.ftp = None
        self.ftp_host = 'arthurhou.pps.eosdis.nasa.gov'
    ...
    def get_datetime_from_file_name(filename):
        ...
        s = ''.join([s[0], s[1][1:]])
        return datetime.datetime.strptime(s, '%Y%m%d%H%M%S') + datetime.timedelta(minutes=15)
        # try:
    ...
    def valid_filename(filename):
        return filename.startswith('3B-HHR.') and filename.endswith('.HDF5')
    ...
    def get_missing_datetime(self, datetime_begin=None, datetime_end=None):
        ...
        for f in fs:
            ...
            if month not in datetime_missing[year]:
                datetime_missing[year][month] = OrderedDict()
            day = f[6:8]
            if day not in datetime_missing[year][month]:
                datetime_missing[year][month][day] = list()
            datetime_missing[year][month][day].append(f)
    ...
    def get_missing_ftp_files(self, missing_datetimes):
        for y, m_dict in missing_datetimes.items():
            for m, d_dict in m_dict.items():
                for d, records in d_dict.items():
                    print y, m, d
                    ftp_dir = '{}{}/{}/{}/{}'.format(self.ftp_dir, y, m, d, 'imerg/')
                    lines = []
                    try:
                        ...
                        if self.verbose and '550' not in e.message:
                            self.print_verbose('{}: {}'.format(ftp_dir, e.message))
